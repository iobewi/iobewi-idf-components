/**
 * @file main.c
 * @brief Exemple de base pour le driver drv_a02yyuw
 *
 * Cet exemple montre comment utiliser le driver pour lire la distance
 * d'un seul capteur A02YYUW avec filtre médian sur 3 mesures.
 */
...
// GPIO EN (STMPS2141STR) - adapte selon ton PCB
static const int EN_PINS[] = {CONFIG_A02_BASIC_EN_GPIO};
...
void app_main(void)
{
    ESP_LOGI(TAG, "Starting A02YYUW basic example");
    ...
    config.uart_rx_gpio = CONFIG_A02_BASIC_UART_RX_GPIO;
    config.uart_tx_gpio = UART_PIN_NO_CHANGE;
    config.baudrate = 9600;
    config.rx_buffer_size = 256;

    config.gpio_mode = CONFIG_A02_BASIC_MODE_GPIO;
    ...
    while (1) {
        ...
        ret = drv_a02yyuw_select(sensor, 0, DRV_A02YYUW_MODE_PROCESSED);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to select sensor: %s", esp_err_to_name(ret));
        ...
        if (e1 == ESP_OK && e2 == ESP_OK && e3 == ESP_OK) {
            ...
            ESP_LOGI(TAG, "Distance = %u mm (raw: %u/%u/%u)",
                     (unsigned)distance,
                     (unsigned)a, (unsigned)b, (unsigned)c);
        } else {
            ESP_LOGW(TAG, "Read failed: %s %s %s",
                     esp_err_to_name(e1),
                     esp_err_to_name(e2),
                     esp_err_to_name(e3));
        }
