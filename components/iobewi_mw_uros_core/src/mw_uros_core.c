...
#define RCCHECK_GOTO(fn, label) {                                                                     \
    ...
    if (temp_rc != RCL_RET_OK) {                                                                      \
        ...
        uros_set_led_state(ctx, LIB_STATUS_LED_ERROR);                                                 \
        goto label;                                                                                   \
    }                                                                                                 \
}
...
static void uros_core_main_task(void *arg)
{
    ...
    while (true) {
        ...
        uros_set_led_state(ctx, LIB_STATUS_LED_WAITING);
        ...
        uros_set_led_state(ctx, LIB_STATUS_LED_CONNECTED);
...
esp_err_t mw_uros_core_destroy(mw_uros_core_t *ctx)
{
    ...
    if (ctx->status_led != NULL) {
        lib_status_led_del(ctx->status_led);
        ctx->status_led = NULL;
    }
...
esp_err_t mw_uros_core_start(mw_uros_core_t *ctx)
{
    ...
    ctx->status_led = NULL;
    if (ctx->config.status_led_gpio >= 0) {
        lib_status_led_config_t led_cfg = {
            .gpio = (gpio_num_t)ctx->config.status_led_gpio,
            .brightness = ctx->config.status_led_brightness,
        };
        esp_err_t ret = lib_status_led_new(&led_cfg, &ctx->status_led);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to create status LED: %s", esp_err_to_name(ret));
            return ret;
        }
        ESP_LOGI(TAG, "Status LED enabled on GPIO %d", ctx->config.status_led_gpio);
    } else {
